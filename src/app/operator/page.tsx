...
function EmployeeManagement() {
  ...
  if (!loading && callerRole !== 'owner') return null;

  return (
    <>
      <div className="g-card p-4 md:p-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h3 className="text-base font-bold" style={{ color: 'var(--sys-text)' }}>従業員管理</h3>
            <p className="text-xs mt-0.5" style={{ color: 'var(--sys-text-muted)' }}>チームメンバーの追加・管理</p>
          </div>
          {!loading && callerRole === 'owner' && (
            <button
              onClick={() => { setShowCreate(true); setCreateError(''); }}
              className="px-3 py-1.5 bg-blue-600 hover:bg-blue-500 text-white font-medium rounded-xl text-xs transition-colors"
            >
              + 従業員追加
            </button>
          )}
        ...
        {loading ? (
          <div className="py-6 text-center text-sm" style={{ color: 'var(--sys-text-muted)' }}>読み込み中...</div>
        ) : (
          <div className="overflow-hidden rounded-xl border border-white/[0.06]">
            <table className="w-full">
              <thead>
                <tr className="border-b border-white/[0.06]" style={{ background: 'rgba(255,255,255,0.02)' }}>
                  <th className="text-left text-xs font-medium px-3 py-2" style={{ color: 'var(--sys-text-muted)' }}>名前</th>
                  <th className="text-left text-xs font-medium px-3 py-2 hidden md:table-cell" style={{ color: 'var(--sys-text-muted)' }}>メール</th>
                  <th className="text-left text-xs font-medium px-3 py-2" style={{ color: 'var(--sys-text-muted)' }}>ロール</th>
                  <th className="text-right text-xs font-medium px-3 py-2" style={{ color: 'var(--sys-text-muted)' }}>操作</th>
                </tr>
              </thead>
              <tbody>
                {users.map((u) => (
                  <tr key={u.id} className="border-b border-white/[0.04] last:border-0">
                    <td className="px-3 py-2.5">
                      <div className="text-sm font-medium" style={{ color: 'var(--sys-text)' }}>{u.display_name}</div>
                      <div className="text-xs md:hidden" style={{ color: 'var(--sys-text-muted)' }}>{u.email}</div>
                    </td>
                    <td className="px-3 py-2.5 text-sm hidden md:table-cell" style={{ color: 'var(--sys-text-muted)' }}>{u.email}</td>
                    <td className="px-3 py-2.5">
                ...
                {users.length === 0 && (
                  <tr>
                    <td colSpan={4} className="px-3 py-6 text-center text-sm" style={{ color: 'var(--sys-text-muted)' }}>
